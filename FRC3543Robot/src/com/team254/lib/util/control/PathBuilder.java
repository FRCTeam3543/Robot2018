...
public class PathBuilder {
    ...
    public static void main(String [] args) {
    		Path path = buildPathFromWaypoints(WaypointListBuilder.create()
    					.add(0, 0, 0, 0, "Origin")
    					.add(100, 100, 0, 5, "First")
    					.add(200, 100, 4, 7, "Second")
    					.add(200, 300, 2, 8, "Third")
    					.toList()
    				);
    ...
    public static class WaypointListBuilder {
    		private List<Waypoint> list = new ArrayList<>();
    		public static WaypointListBuilder create() {
    			return new WaypointListBuilder();
    		}
    		public WaypointListBuilder add(double x, double y, double r, double s, String m) {
    			list.add(new Waypoint(x,y,r,s,m));
    			return this;
    		}
            ...
            if (pathLength > kEpsilon) {
                if (b.marker != null) {
                    p.addSegment(new PathSegment(start.x(), start.y(), end.x(), end.y(), b.speed,
                            PathConstants.kPathFollowingMaxAccel, p.getLastMotionState(), endSpeed, b.marker));
                } else {
                    p.addSegment(new PathSegment(start.x(), start.y(), end.x(), end.y(), b.speed,
                            PathConstants.kPathFollowingMaxAccel, p.getLastMotionState(), endSpeed));
                }
            }
        ...
        private void addToPath(Path p) {
            ...
            if (radius > kEpsilon && radius < kReallyBigNumber) {
                p.addSegment(new PathSegment(a.end.x(), a.end.y(), b.start.x(), b.start.y(), center.x(), center.y(),
                        speed, PathConstants.kPathFollowingMaxAccel, p.getLastMotionState(), b.speed));
            }
        }
