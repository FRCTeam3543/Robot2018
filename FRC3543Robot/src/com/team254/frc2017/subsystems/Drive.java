...
import com.team254.lib.util.drivers.WPI_TalonSRXFactory;
...
/**
 * This subsystem is based on the Team 254 2017, but adapted for Command robot and CTRE Phoenix.
 * 
 ...
 * @see Subsystem.java
 */
public class Drive extends BaseSubsystem {
...
//    };
    ...
    private Drive() {
        ...
        mLeftMaster = WPI_TalonSRXFactory.createDefaultTalon(Constants.kLeftDriveMasterId);
...
//        }
//
        mLeftSlave = WPI_TalonSRXFactory.createPermanentSlaveTalon(Constants.kLeftDriveSlaveId, mLeftMaster);
//                Constants.kLeftDriveMasterId);
//        mLeftSlave.reverseOutput(false);
//        mLeftMaster.setStatusFrameRateMs(StatusFrameRate.Feedback, 5);

        mRightMaster = WPI_TalonSRXFactory.createDefaultTalon(Constants.kRightDriveMasterId);
...
//        }
//
        mRightSlave = WPI_TalonSRXFactory.createPermanentSlaveTalon(Constants.kRightDriverSlaveId, mRightMaster);
...
////                    rpmToInchesPerSecond(mLeftMaster.getClosedLoopTarget()) - left_speed);
////            SmartDashboard.putNumber("right speed error (ips)",
////                    rpmToInchesPerSecond(mRightMaster.getClosedLoopTarget()) - right_speed);
//        } else {
...
//            }
//        }
//        SmartDashboard.putNumber("left position (rotations)", mLeftMaster.getSelectedSensorPosition(kLowGearPositionControlSlot));
//        SmartDashboard.putNumber("right position (rotations)", mRightMaster.getSelectedSensorPosition(kLowGearPositionControlSlot));
...
//    }
    ...
    public synchronized void resetEncoders() {
        mLeftMaster.getSensorCollection().setQuadraturePosition(0, kMS);        
        mLeftMaster.setSelectedSensorPosition(0, kLowGearPositionControlSlot, kMS);
        mRightMaster.setSelectedSensorPosition(0, kLowGearPositionControlSlot, kMS);
        mRightMaster.getSensorCollection().setQuadraturePosition(0, kMS);
        mLeftMaster.setSelectedSensorPosition(0, kLowGearPositionControlSlot, kMS);
        mRightSlave.setSelectedSensorPosition(0, kLowGearPositionControlSlot, kMS);        
    }
...
//                    -Constants.kDriveLowGearNominalOutput);
    ...
    static double inchesPerSecondToRpm(double inches_per_second) {
        return inchesToRotations(inches_per_second) * 60;
    }

    public double getLeftDistanceInches() {
        return rotationsToInches(mLeftMaster.getSensorCollection().getQuadraturePosition());
    }

    public double getRightDistanceInches() {
        return rotationsToInches(mRightMaster.getSensorCollection().getQuadraturePosition());
    }

    public double getLeftVelocityInchesPerSec() {
        return rpmToInchesPerSecond(mLeftMaster.getSensorCollection().getQuadratureVelocity());
    }

    public double getRightVelocityInchesPerSec() {
        return rpmToInchesPerSecond(mRightMaster.getSensorCollection().getQuadratureVelocity());
    }
...
//        mRightSlave.set(ControlMode.Velocity, 0.0);
        mLeftMaster.set(ControlMode.Velocity, 0.0);
//        mLeftSlave.set(ControlMode.Velocity, 0.0);

        mRightMaster.set(-6.0f);
        Timer.delay(4.0);
        final double currentRightMaster = mRightMaster.getOutputCurrent();
        final double rpmRightMaster = mRightMaster.getSelectedSensorVelocity(kHighGearVelocityControlSlot);
        ...
        final double rpmRightSlave = mRightMaster.getSelectedSensorVelocity(kHighGearVelocityControlSlot);
        ...
        final double rpmLeftMaster = mLeftMaster.getSelectedSensorVelocity(kHighGearVelocityControlSlot);
        ...
        final double rpmLeftSlave = mLeftMaster.getSelectedSensorVelocity(kHighGearVelocityControlSlot);
        mLeftSlave.set(0.0);
